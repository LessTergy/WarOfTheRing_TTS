    ...
    for root, _, files in os.walk(directory_path):
        for file_name in files:
            if file_name.endswith(".json"):
                ...
                name = extract_name_and_symbols(file_name)
                type_in_description = extract_description_word(full_file_path, pattern)
...
def extract_name_and_symbols(file_name):
    name, _ = file_name.split(".", 1)
    return name


def extract_description_word(file_path, pattern):
