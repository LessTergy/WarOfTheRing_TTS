...
function ProcessNextStep()
    ...
    elseif Step == "AlternateCardsMenu" then
        self.clearButtons()
        self.createButton(
            {
                click_function = "Nothing",
                function_owner = self,
                label = "Art/Alternate Cards?",
                position = {0, 0.1, -1.3},
                width = 0,
                height = 0,
                font_size = 150,
        ...
        self.createButton(
            {
                click_function = "Nothing",
                function_owner = self,
                label = "(Character & Strategy Cards)",
                position = {0, 0.1, -1},
                width = 0,
                height = 0,
                font_size = 100,
        ...
        if not ArtAlternateCards then
            self.createButton(
                {
                    click_function = "SelectStandardCards",
                    function_owner = self,
                    label = "Selected: Standard Cards",
                    position = {0, 0.1, -0.6},
                    width = 1800,
                    height = 150,
                    color = {1, 1, 0},
                    font_size = 80,
                    tooltip = "Play with the Standard Character and Strategy Cards from the original War of the Ring Box."
                }
            )
            self.createButton(
                {
                    click_function = "SelectAlternateCards",
                    function_owner = self,
                    label = "Art/Alternate Cards",
                    position = {0, 0.1, -0.3},
                    width = 1800,
                    ...
                    color = {1, 1, 1},
                    font_size = 80,
                    tooltip = "Play with the Art/Alternate Character and Strategy Cards from Michel."
                }
            )
        else --standard cards...
            self.createButton(
            ...
            self.createButton(
                {
                    click_function = "SelectAlternateCards",
                    function_owner = self,
                    label = "Selected: Art/Alternate Cards",
                    position = {0, 0.1, -0.3},
                    width = 1800,
                    height = 150,
                    color = {1, 1, 0},
        ...
        self.createButton(
            {
                ...
                color = {1, 1, 1},
                font_size = 100,
                tooltip = "Continue Setup with the currently selected Character/Strategy Card option."
            }
        )
        function SelectStandardCards()
            self.clearButtons()
            ArtAlternateCards = false
            Step = ""
        end

        function SelectAlternateCards()
            self.clearButtons()
            ArtAlternateCards = true
            Step = ""
        end

        function Continue()
            self.clearButtons()
            if ArtAlternateCards then
                --swap shadow character deck...
                if getObjectFromGUID("6d8b6f") ~= nil then
                    getObjectFromGUID("6d8b6f").setPositionSmooth(
                        getObjectFromGUID(IDs.ShadowCharacterEventDeck).getPosition(),
                        false,
                        false
                    )
                    getObjectFromGUID("6d8b6f").setRotationSmooth(
                        getObjectFromGUID(IDs.ShadowCharacterEventDeck).getRotation(),
                        false,
                        false
                    )
                    getObjectFromGUID(IDs.ShadowCharacterEventDeck).destruct()
                    IDs.ShadowCharacterEventDeck = "6d8b6f"
                else
                    printToAll("Could not find Art/Alternate Shadow Character Deck.")
                end

                --swap shadow strategy deck...
                if getObjectFromGUID("f429d8") ~= nil then
                    getObjectFromGUID("f429d8").setPositionSmooth(
                        getObjectFromGUID(IDs.ShadowStrategyEventDeck).getPosition(),
                        false,
                        false
                    )
                    getObjectFromGUID("f429d8").setRotationSmooth(
                        getObjectFromGUID(IDs.ShadowStrategyEventDeck).getRotation(),
                        false,
                        false
                    )
                    getObjectFromGUID(IDs.ShadowStrategyEventDeck).destruct()
                    IDs.ShadowStrategyEventDeck = "f429d8"
                else
                    printToAll("Could not find Art/Alternate Shadow Strategy Deck.")
                end

                --swap FP character deck...
                if getObjectFromGUID("992fc5") ~= nil then
                    getObjectFromGUID("992fc5").setPositionSmooth(
                        getObjectFromGUID(IDs.FreePeoplesCharacterEventDeck).getPosition(),
                        false,
                        false
                    )
                    getObjectFromGUID("992fc5").setRotationSmooth(
                        getObjectFromGUID(IDs.FreePeoplesCharacterEventDeck).getRotation(),
                        false,
                        false
                    )
                    getObjectFromGUID(IDs.FreePeoplesCharacterEventDeck).destruct()
                    IDs.FreePeoplesCharacterEventDeck = "992fc5"
                else
                    printToAll("Could not find Art/Alternate Free Peoples Character Deck.")
                end

                --swap FP strategy deck...
                if getObjectFromGUID("d3fdbc") ~= nil then
                    getObjectFromGUID("d3fdbc").setPositionSmooth(
                        getObjectFromGUID(IDs.FreePeoplesStrategyEventDeck).getPosition(),
                        false,
                        false
                    )
                    getObjectFromGUID("d3fdbc").setRotationSmooth(
                        getObjectFromGUID(IDs.FreePeoplesStrategyEventDeck).getRotation(),
                        false,
                        false
                    )
                    getObjectFromGUID(IDs.FreePeoplesStrategyEventDeck).destruct()
                    IDs.FreePeoplesStrategyEventDeck = "d3fdbc"
                else
                    printToAll("Could not find Art/Alternate Free Peoples Strategy Deck.")
                end

                --swap shadow faction deck...
                if getObjectFromGUID("e25d28") ~= nil then
                    getObjectFromGUID("e25d28").setPositionSmooth(
                        getObjectFromGUID(IDs.WoME.ShadowFactionDeck).getPosition(),
                        false,
                        false
                    )
                    getObjectFromGUID("e25d28").setRotationSmooth(
                        getObjectFromGUID(IDs.WoME.ShadowFactionDeck).getRotation(),
                        false,
                        false
                    )
                    getObjectFromGUID(IDs.WoME.ShadowFactionDeck).destruct()
                    IDs.WoME.ShadowFactionDeck = "e25d28"
                else
                    printToAll("Could not find Art/Alternate Shadow Faction Deck.")
                end

                --swap FP faction deck...
                if getObjectFromGUID("760c83") ~= nil then
                    getObjectFromGUID("760c83").setPositionSmooth(
                        getObjectFromGUID(IDs.WoME.FreePeoplesFactionDeck).getPosition(),
                        false,
                        false
                    )
                    getObjectFromGUID("760c83").setRotationSmooth(
                        getObjectFromGUID(IDs.WoME.FreePeoplesFactionDeck).getRotation(),
                        false,
                        false
                    )
                    getObjectFromGUID(IDs.WoME.FreePeoplesFactionDeck).destruct()
                    IDs.WoME.FreePeoplesFactionDeck = "760c83"
                else
                    printToAll("Could not find Art/Alternate Free Peoples Faction Deck.")
                end

                --swap shadow call to battle deck...
                if getObjectFromGUID("f1c67e") ~= nil then
                    getObjectFromGUID("f1c67e").setPositionSmooth(
                        getObjectFromGUID(IDs.WoME.ShadowCallToBattleDeck).getPosition(),
                        false,
                        false
                    )
                    getObjectFromGUID("f1c67e").setRotationSmooth(
                        getObjectFromGUID(IDs.WoME.ShadowCallToBattleDeck).getRotation(),
                        false,
                        false
                    )
                    getObjectFromGUID(IDs.WoME.ShadowCallToBattleDeck).destruct()
                    IDs.WoME.ShadowCallToBattleDeck = "f1c67e"
                else
                    printToAll("Could not find Art/Alternate Shadow call to battle Deck.")
                end

                --swap FP call to battle deck...
                if getObjectFromGUID("20b770") ~= nil then
                    getObjectFromGUID("20b770").setPositionSmooth(
                        getObjectFromGUID(IDs.WoME.FreePeoplesCallToBattleDeck).getPosition(),
                        false,
                        false
                    )
                    getObjectFromGUID("20b770").setRotationSmooth(
                        getObjectFromGUID(IDs.WoME.FreePeoplesCallToBattleDeck).getRotation(),
                        false,
                        false
                    )
                    getObjectFromGUID(IDs.WoME.FreePeoplesCallToBattleDeck).destruct()
                    IDs.WoME.FreePeoplesCallToBattleDeck = "20b770"
                else
                    printToAll("Could not find Art/Alternate Free Peoples call to battle Deck.")
                end
            else --standard decks...
                --remove alternate art cards...
                if getObjectFromGUID("6d8b6f") ~= nil then
                    getObjectFromGUID("416864").putObject(getObjectFromGUID("6d8b6f"))
                end

                if getObjectFromGUID("f429d8") ~= nil then
                    getObjectFromGUID("416864").putObject(getObjectFromGUID("f429d8"))
                end

                if getObjectFromGUID("992fc5") ~= nil then
                    getObjectFromGUID("416864").putObject(getObjectFromGUID("992fc5"))
                end

                if getObjectFromGUID("d3fdbc") ~= nil then
                    getObjectFromGUID("416864").putObject(getObjectFromGUID("d3fdbc"))
                end

                if getObjectFromGUID("e25d28") ~= nil then
                    getObjectFromGUID("416864").putObject(getObjectFromGUID("e25d28"))
                end

                if getObjectFromGUID("f1c67e") ~= nil then
                    getObjectFromGUID("416864").putObject(getObjectFromGUID("f1c67e"))
                end

                if getObjectFromGUID("760c83") ~= nil then
                    getObjectFromGUID("416864").putObject(getObjectFromGUID("760c83"))
                end

                if getObjectFromGUID("20b770") ~= nil then
                    getObjectFromGUID("416864").putObject(getObjectFromGUID("20b770"))
                end
            end
